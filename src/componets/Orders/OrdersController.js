...
const { ResponseSchema, MergeImageLink,MergePDfLink, PaginateSchema } = require('../../helper/HelperFunctions');
...
const { AddOrder, GetOrderCount, GetOrdersByQueryPaginated, GetOrderByQuery, AddSpecialOrder, 
  GetSpecialOrderById, UpdateSpecialOrder, UpdateSpecialOrderSession, AddOrderSession, GetSpecialOrderByIdPopulated, 
  ...
  DeleteOrder,
  GetOrderById} = require('./OrdersService');
const { GetProductById, GetProductBySlug } = require('../Products/ProductsService');
...
exports.createOrder = async (req, res) => {
  ...
  let subTotalPrice=0
  let totalPrice=0
  await Promise.all(products?.map(async (product) => new Promise(async (resolve, reject) => {
    ...
    if( product?.selected_size_id !='undefined'){
      size =productItem?.sizes?.filter(size=>{
        return size?.id == product?.selected_size_id
      })
      price = size?.[0]?.price
    }else{
  ...
  try {
    ...
    try {
      await new Promise((resolve, reject)=>{
        let addedInfo ={
          ...
          created_at: moment(addedOrder?.created_at).format('MMMM Do YYYY, h:mm:ss a'),
        }
        
        pdf.create(OrderPdf(addedInfo), {}).toFile(`./public/files/Order.No-${addedOrder?.item_number}-invoice.pdf`, (err) => {
          if(err) {
            console.log(err)
          }
          resolve()
        ...
        if(billing_address_email){
          SendMails(billing_address_email,`Order #${addedOrder?.item_number} Created Successfully`,CREATE_EMAIL_HTML(MergePDfLink(req,`Order.No-${addedOrder?.item_number}-invoice.pdf`),addedOrder?.billing_address?.name,addedOrder?.item_number))
        }
        if(generalSetting?.project_email_address){
          SendMails(generalSetting?.project_email_address,`Order #${addedOrder?.item_number} Created Successfully`,CREATE_EMAIL_HTML(MergePDfLink(req,`Order.No-${addedOrder?.item_number}-invoice.pdf`),'Admin',addedOrder?.item_number))
        }
    });
    } catch (error) {
      console.log(error)
    ...
    return res.status(200).json(ResponseSchema(req.t('Order Added Successfully'), true,addedOrder))
  } catch (error) {
    console.log(error)
...
exports.getAllUsersOrders = async (req, res) => {
  ...
  try {
    ...
    const sendedObject = orders.map((order) => ({
      ...
      invoice_file:order?.item_number&&MergePDfLink(req,`Order.No-${order?.item_number}-invoice.pdf`),
      created_at: moment(order?.created_at).format('MMMM Do YYYY, h:mm:ss a'),
    }));
    return res.status(200).json(ResponseSchema(req.t('Orders'), true, sendedObject));
  } catch (error) {
    console.log(error)
    return res.status(400).json(ResponseSchema(req.t('Somethings Went wrong'), false, ErrorHandler(error)))
  }
...
exports.getAllOrders = async (req, res) => {
  ...
  try {
    let orders = await GetAllOrdersPaginatedFiltered(queryParams,page,itemPerPage)
    const sendedObject = orders.map((order) => ({
      id: order?._id,
      item_number: order?.item_number,
      status: order?.status,
    ...
    return res.status(200).json(ResponseSchema(req.t('Orders'), true, PaginateSchema(page + 1, pages, count, sendedObject)));
  } catch (error) {
    console.log(error)
    return res.status(400).json(ResponseSchema(req.t('Somethings Went wrong'), false, ErrorHandler(error)))
  }
...
exports.getAllOrdersWithStatus = async (req, res) => {
  ...
  if(authedUser?.user_type=='user'){
    ...
    return res.status(400).json(ResponseSchema(req.t('Token Is Required'), false));
  }
  ...
  try {
    ...
    const sendedObject = orders.map((order) => ({
      ...
      invoice_file:order?.item_number&&MergePDfLink(req,`Order.No-${order?.item_number}-invoice.pdf`),
      created_at: moment(order?.created_at).locale((authedUser?.user_type=='user'?order?.user_id?.current_language:order?.provider_id?.current_language)||'en').format('MMMM Do YYYY, h:mm:ss a'),
    }));
    return res.status(200).json(ResponseSchema(req.t('Orders'), true, PaginateSchema(page + 1, pages, count, sendedObject)));
  } catch (error) {
    console.log(error)
...
exports.getOrder = async (req, res) => {
 ...
 const lang = req.headers['accept-language']?req.headers['accept-language']: 'en';
  ...
  try {
    ...
    return res.status(200).json(ResponseSchema(req.t('Order'), true, sendedObject));
  } catch (error) {
    console.log(error)
    return res.status(400).json(ResponseSchema(req.t('Somethings Went wrong'), false, ErrorHandler(error)))
  }
...
exports.changeOrderStatus = async (req, res) => {
  ...
  if (!CheckValidIdObject(req, res, order_id, req.t('Product Id is Invalid'))) return;
  
  let order = await GetOrderByQuery({_id : order_id})
  if(!order){
    return res.status(400).json(ResponseSchema(req.t('Order Doesn\'t Exist'), false))
  ...
  try {
    ...
    return res.status(200).json(ResponseSchema(req.t('Order Status Updated Successfully'), true  ))
  } catch (error) {
    console.log(error)
    return res.status(400).json(ResponseSchema(req.t('Somethings Went wrong'), false, ErrorHandler(error)))
  }
...
exports.payOrder = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json(ResponseSchema(req.t('Order Paid Successfully'), true  ))
  } catch (error) {
    console.log(error)
    return res.status(400).json(ResponseSchema(req.t('Somethings Went wrong'), false, ErrorHandler(error)))
  }
...
exports.askForSpecialRequestOrder = async (req, res) => {
  ...
  const user = await GetUserById(authedUser?.user_id);
  if (!user) {
    return res.status(400).json(ResponseSchema(req.t("You don't have permission to create order"), false));
  }
  ...
  if (!provider) {
    return res.status(400).json(ResponseSchema(req.t('Provider Id is wrong'), false));
  }
  if(provider?.has_special_requests==2){
    return res.status(403).json(ResponseSchema(req.t('This Provider Doesn\'t Offer Special Requests'), false));
  }
  try {
    let addedData = {
      user_id: authedUser?.user_id,
      provider_id: provider_id,
      special_request_audio_file: files?.special_request_audio_file?.[0]?.filename,
    }
    ...
    return res.status(200).json(ResponseSchema(req.t('special Request Order Added Successfully'), true  ))
  } catch (error) {
    console.log(error)
    return res.status(400).json(ResponseSchema(req.t('Somethings Went wrong'), false, ErrorHandler(error)))
  }
...
exports.sendMessageInSpecialRequest = async (req, res) => {
  ...
  if (authedUser?.user_id != specialRequest?.user_id?._id && authedUser?.user_id != specialRequest?.provider_id?._id) {
    return res.status(400).json(ResponseSchema(req.t("You don't have permission to add message to this request"), false));
  }

  if (message_type == '1' && !message) {
    return res.status(400).json(ResponseSchema(req.t('Message Is Required'), false));
  }
  if (message_type != '1' && !files?.uploaded_message_file) {
    return res.status(400).json(ResponseSchema(req.t('Message File Is Required'), false));
  }
  try {
    let updatedData ={
      $push: {
    ...
    try {
      let addedNotificationData ={
        user_id: authedUser?.user_type=='user'? null:specialRequest?.user_id?._id,
        provider_id: authedUser?.user_type=='provider'? null:specialRequest?.provider_id?._id,
        translation:{
          en:{
            title:'New Message',
            body:`${authedUser?.user_type=='user'?specialRequest?.user_id?.name:specialRequest?.provider_id?.store_name} Sends To You New Message`
          },
          ar:{
            title:'رسالة جديدة',
            body:`${authedUser?.user_type=='user'?specialRequest?.user_id?.name:specialRequest?.provider_id?.store_name} يرسل لك رسالة جديد`
          },
          ur:{
            title:'نیا پیغام',
            body:`${authedUser?.user_type=='user'?specialRequest?.user_id?.name:specialRequest?.provider_id?.store_name} آپ کو نیا پیغام بھیجتا ہے۔`
          },
    ...
    return res.status(200).json(ResponseSchema(req.t('special Request message Added Successfully'), true))
  } catch (error) {
    console.log(error)
    return res.status(400).json(ResponseSchema(req.t('Somethings Went wrong'), false, ErrorHandler(error)))
  }
...
exports.sendOfferToSpecialRequest = async (req, res) => {
  ...
  if (authedUser?.user_type != 'provider') {
    return res.status(403).json(ResponseSchema(req.t("You don't have permission to add offer to this request"), false));
  }
  const specialRequest  =await GetSpecialOrderByIdPopulated(special_request_id)
  ...
  try {
    ...
    return res.status(200).json(ResponseSchema(req.t('special Request Offer Sended Successfully'), true))
  } catch (error) {
    return res.status(400).json(ResponseSchema(req.t('Somethings Went wrong'), false, ErrorHandler(error)))
  }
};
...
exports.acceptSpecialRequestOffer = async (req, res) => {
  ...
  if (authedUser?.user_type != 'user') {
    return res.status(403).json(ResponseSchema(req.t("You don't have permission to accept offer to this request"), false));
  }
  const specialRequest = await GetSpecialOrderByIdPopulated(special_request_id)
  ...
  if (specialRequest?.status == 2) {
    return res.status(403).json(ResponseSchema(req.t('Request Already Accepted'), false));
  }
  try {
    ...
    return res.status(200).json(ResponseSchema(req.t('Special Request has been Accepted Successfully.'), true));
  } catch (error) {
...
exports.getSpecialRequestMessages = async (req, res) => {
  ...
  if (!CheckValidIdObject(req, res, id, req.t('Special Request Id is Invalid'))) return;

  const specialRequest = await GetSpecialOrderById(id);
  if (!specialRequest) {
    return res.status(400).json(ResponseSchema(req.t('Special Request Id is wrong'), false));
  }
  if (authedUser?.user_id != specialRequest?.user_id && authedUser?.user_id != specialRequest?.provider_id) {
    return res.status(400).json(ResponseSchema(req.t("You don't have permission to show messages to this request"), false));
  }
  try {
    ...
    const sendedMessage = order?.messages.map((message) => {
      ...
      if (message?.message_type == '1') {
        ...
        messageObj.message = message?.uploaded_message_file && MergeImageLink(req, message?.uploaded_message_file);
      }
      return messageObj;
    });
    const {
      _id, status, order_type, special_request_audio_file,
    ...
    const sendedObject = {
      ...
      special_request_audio_file: special_request_audio_file&& MergeImageLink(req, special_request_audio_file),
...
exports.getAllSpecialRequests = async (req, res) => {
  ...
  try {
    ...
    return{
      ...
      provider_image: personal_photo && MergeImageLink(req, personal_photo),
      special_request_audio_file: special_request_audio_file&& MergeImageLink(req, special_request_audio_file),
      created_at: moment(order?.created_at).locale(authedUser?.user_type=='user'?order?.user_id?.current_language:order?.provider_id?.current_language).format('MMMM Do YYYY, h:mm:ss a'),
    };
  })
...
exports.deleteOrder = async (req, res) => {
  ...
  try {
    ...
    return res.status(201).json(ResponseSchema(req.t('Order Deleted Successfully'), true))
  } catch (error) {
    console.log(error)
    return res.status(400).json(ResponseSchema(req.t('Somethings Went wrong'), false, ErrorHandler(error)))
  }
