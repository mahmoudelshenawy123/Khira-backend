...
const {
  createOrder,  getAllProvidersOrdersWithStatus,askForSpecialRequestOrder, sendMessageInSpecialRequest,
  getSpecialRequestMessages,sendOfferToSpecialRequest, acceptSpecialRequestOffer, getAllOrdersWithStatus, 
  getOrder,
...
const upload = multer({
  ...
  { name: 'special_request_audio_file', maxCount: 1 },
]);
...
const uploadMessageFile = multer({
  ...
  { name: 'uploaded_message_file', maxCount: 1 },
]);
...
router.post('/create-order', multer().none(), createOrder);
...
router.post('/ask-for-special-request', uploadyModififed, askForSpecialRequestOrder);

router.post('/send-message', uploadMessageFileModififed, sendMessageInSpecialRequest);

router.post('/send-offer-to-special-request', multer().none(), sendOfferToSpecialRequest);

router.post('/accept-special-request-offer', multer().none(), acceptSpecialRequestOffer);
