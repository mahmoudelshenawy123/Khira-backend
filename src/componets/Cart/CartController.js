...
const {
  ...
  UpdateCartItem,
  GetCartItems,
  GetAllCartItemsPopulated,
  GetCartItemsCount,
  GetAllCartItemsPopulatedAndPaginated,
  DeleteCartItem,
  GetAllCartItems,
} = require('./CartService')
const {
...
exports.addProductToCart = async (req, res) => {
  ...
  if (!CheckValidIdObject(req, res, product_id, req.t('Product Id is Invalid')))
    return
  ...
  const cartItem = await GetAllCartItems(cartItemQuery)
  const isCartItemExists =
    cartItem &&
    cartItem?.find((item) => item.selected_size_id === selected_size_id)
  if (isCartItemExists) {
    try {
      addedCartItem = {
        ...addedCartItem,
        quantity: Number(quantity) + Number(isCartItemExists?.quantity),
      }
      let addedItem = await UpdateCartItem(isCartItemExists?._id, addedCartItem)

      let cartData = await getAllCartItemsData(
        authedUser?.user_id,
        unique_identifier,
        req
      )
      ...
      return res
        ...
        .json(
          ResponseSchema(
            req.t('Product Updated In Cart Successfully'),
            true,
            cartData
          )
        )
    ...
    try {
      ...
      return res
        ...
        .json(
          ResponseSchema(
            req.t('Product Added To Cart Successfully'),
            true,
            cartData
          )
        )
...
exports.updateCartItemQuantity = async (req, res) => {
  ...
  try {
    ...
    cart_item_id &&
      (await Promise.all(
        cart_item_id?.map(async (item, index) => {
          if (
            !CheckValidIdObject(
              req,
              res,
              item,
              req.t('Cart Item Id is Invalid')
            )
          )
            return
  ...
  try {
    ...
    const sendedProductObject = cartItems.map((item) => {
      ...
      return {
        ...
        price: item?.is_gift
          ? price + generalSetting?.wrap_as_gift_value
          : price,
    ...
    return res
      .status(200)
      .json(ResponseSchema(req.t('Cart Items'), true, sendedObject))
  } catch (error) {
    console.log(error)
    return res
      .status(400)
...
exports.getAllCartItemsWithPagination = async (req, res) => {
  ...
  try {
    let cartItems = await GetAllCartItemsPopulatedAndPaginated(
      findQuery,
      page,
      itemPerPage
    )
    const sendedObject = cartItems.map((item) => ({
      ...
      product_image: item?.product_id?.images?.[0]
        ? MergeImageLink(req, item?.product_id?.images[0])
        : '',
      product_price: Number(
        (
          item?.product_id?.price_before_discount -
          item?.product_id?.price_before_discount *
...
exports.deleteCartItem = async (req, res) => {
  ...
  if (!cartItem) {
    return res
      .status(400)
      .json(ResponseSchema(req.t('Cart Item Id is wrong'), false))
  }
  try {
    ...
    return res
      .status(201)
      .json(ResponseSchema(req.t('Cart Deleted Successfully'), true, cartData))
  } catch (error) {
    return res
      .status(400)
      .json(
