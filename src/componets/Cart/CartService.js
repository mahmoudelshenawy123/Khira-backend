...
exports.GetAllCartItems = async (query) => {
  try {
    let cartItem = await Cart.find(query)
    return cartItem
  } catch (err) {
    throw err
  }
...
exports.GetAllCartItemsPopulatedAndPaginated = async (
  query,
  page,
  itemPerPage
) => {
  try {
    let cartItems = await Cart.find(query)
      .populate(['product_id', 'provider_id'])
      .sort({ _id: -1 })
